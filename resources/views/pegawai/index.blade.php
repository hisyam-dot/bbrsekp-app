...
<section class="max-w-7xl mx-auto px-6 py-12">
            ...
            <div class="w-[30%] relative">
                ...
                <div id="dropdownPopuler" class="hidden absolute z-50 mt-2 w-full bg-white border rounded-lg shadow-lg overflow-hidden">
                    ...
                    @foreach($populer as $item)
                        <a href="{{ route('pegawai.index', ['q' => $item->keyword]) }}"
    ...
    @if($search)
        <div class="mb-6">
            <span class="inline-block bg-blue-50 px-4 py-2 rounded-lg text-sm shadow-sm font-semibold text-blue-700">
                {{ $details->total() }}
            </span> data ditemukan untuk pencarian
            "<span class="italic">{{ ucfirst($search) }}</span>"
        </div>
    @endif
